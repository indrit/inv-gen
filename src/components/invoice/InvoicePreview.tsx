...
export function InvoicePreview({ data, logo, isPremium = false, watermark = false }: InvoicePreviewProps) {
  ...
  return (
    <div className="bg-white p-12 text-black font-sans max-w-[850px] mx-auto border shadow-sm relative overflow-hidden" id="invoice-pdf-content">
      {watermark && (
      ...
      <div className="flex justify-end mb-12">
        <div className="w-full max-w-[300px] space-y-2">
          ...
          {data.paymentTerms && (
            <div className="grid grid-cols-2 gap-4 text-sm">
              <span className="text-right text-gray-500">Payment Terms</span>
              <span className="text-right">{data.paymentTerms}</span>
            </div>
          )}
          <div className="grid grid-cols-2 gap-4 text-sm">
            <span className="text-right text-gray-500">Due Date</span>
            <span className="text-right">{isMounted && format(data.dueDate, 'yyyy-MM-dd')}</span>
          </div>
          {data.poNumber && (
            <div className="grid grid-cols-2 gap-4 text-sm">
              <span className="text-right text-gray-500">PO Number</span>
              <span className="text-right">{data.poNumber}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-[1fr_300px] gap-12 mb-12">
        <div className="space-y-8">
          {data.notes && (
            <div>
              <h2 className="text-sm font-normal text-gray-500 mb-2">Notes</h2>
              <p className="text-sm text-gray-700 whitespace-pre-wrap">{data.notes}</p>
            </div>
          )}
          {data.terms && (
            <div>
              <h2 className="text-sm font-normal text-gray-500 mb-2">Terms</h2>
        ...
        <div className="space-y-3">
          ...
          {data.discount !== undefined && data.discount > 0 && (
            <div className="flex justify-between text-sm">
              ...
              <span>-{currentCurrency.symbol}{data.discount.toFixed(2)}</span>
            </div>
          )}
          {data.shipping !== undefined && data.shipping > 0 && (
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Shipping</span>
              <span>{currentCurrency.symbol}{data.shipping.toFixed(2)}</span>
            </div>
          )}
          ...
          {data.amountPaid !== undefined && data.amountPaid > 0 && (
            <div className="flex justify-between text-sm">
              ...
              <span>{currentCurrency.symbol}{data.amountPaid.toFixed(2)}</span>
            </div>
          )}
          <Separator className="my-2" />
          <div className="flex justify-between font-bold text-xl pt-1">
            <span>Balance Due</span>
            <span>{currentCurrency.symbol}{balanceDue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
        </div>
      </div>
